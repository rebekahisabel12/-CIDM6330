...
class BorrowedBooksViewTest(TestCase):
    def setUp(self):
        test_user1 = User.objects.create_user(
    ...
    def test_view_restricted_to_superuser_or_staff(self):
        # Test access for a regular user
        # self.client.login(username='regular_user', password='testpass')
        response = self.client.get(reverse('all-borrowed'))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('all-borrowed'))

# #         # Test access for a superuser
# #         self.client.login(username='admin', password='adminpass')
#         response = self.client.get(reverse('all-borrowed'))
#         # self.assertRedirects(response)
#         self.assertEqual(response.status_code, 200)

#     # def test_page_displays_borrowed_books_info(self):
...
#     #     # Check if the borrowed book information is displayed
#     #     self.assertContains(response, 'Test Book')
#     #     self.assertContains(response, self.borrowed_book_instance.due_back)
#     #     # User who checked it out
#     #     self.assertContains(response, 'regular_user')

#     #     # Check for the option to renew the due date
#     #     self.assertContains(response, 'Renew Due Date')
